...
def analisis_univariable_numericas(df, features_num):
    ...
    for col in features_num:
        ...
        log_transform = False
        if curtosis_val > 3 or asimetria > 1.2 or outliers_superiores_pro > 5 or outliers_inferiores_pro < 5 or jb_p_value < 0.05:
            log_transform = True
            print(f'La variable {col} puede necesitar una transformación logarítmica debido a heavy tail:')
            print(f'curtosis={curtosis_val:.2f}, asimetria={asimetria}, outliers_sup.={outliers_superiores_pro}, outliers_inf={outliers_inferiores_pro}, JB p-value={jb_res:.4f}')
            print()
...
def analisis_bivariable_numericas_numerica(df, target, features_num):
    ...
    # Análisis de multicolinealidad para variables significativas
    if significant_vars:
...
# target = 'nombre_de_tu_variable_target'  # Tu variable objetivo
# features_num = ['feature1', 'feature2', 'feature3']  # Lista de tus variables numéricas
# analisis_bivariable_numericas_target(df, target, features_num)
